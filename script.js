...
if(process.argv[2] === '--info' || process.argv[2] === '-i'){
  console.log(exec('lipo -info WebRTC',{cwd:MODULE_BIN_PATH}).toString().trim());
}
...
if(process.argv[2] === '--device' || process.argv[2] === '-d'){
  ...
  exec(`lipo -o WebRTC -create WebRTC-armv7 WebRTC-arm64`,{cwd:MODULE_BIN_PATH});
  console.log(exec('ls -ahl | grep WebRTC',{cwd:MODULE_BIN_PATH}).toString().trim());
  console.log('Done!');
}
